...
def load_run_config(run_dir):
    ...
    arch = cfg.get("arch", "mlp")              # "mlp" or "gru"
    rnn_hidden = int(cfg.get("rnn_hidden", 147))

    return env_id, entry_point, env_kwargs, arch, rnn_hidden


def load_model(ckpt_path, obs_dim, act_dim, device, arch="mlp", rnn_hidden=147):
    if arch == "gru":
...
@torch.no_grad()
def rollout_trajectories(env_id, env_kwargs, model, device, arch='mlp', episodes=20, seed_base=100000):
    ...
    for i in range(episodes):
        ...
        h = None  # <-- RNN hidden (episode 시작마다 reset)
...
def main():
    ...
    env_id, entry_point, env_kwargs, arch, rnn_hidden = load_run_config(args.run_dir)
    ...
    for tag, ckpt_path in ckpts:
        ...
        model = load_model(ckpt_path, obs_dim, act_dim, device, arch=arch, rnn_hidden=rnn_hidden)
        trajs, L, r_goal, sigma_c, bg_c, wind_x, wind_y = rollout_trajectories(
            env_id, env_kwargs,model, device, arch=arch, episodes=args.episodes, seed_base=args.seed_base
        )
