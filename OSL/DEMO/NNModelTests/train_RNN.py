...
# =======================
# Recurrent Replay Buffer (episode -> sequence sampling)
# =======================
class EpisodeReplayBuffer:
    def __init__(self, cap_steps=150000):
        self.cap_steps = int(cap_steps)
        self.episodes = []   # list[list[(obs, a, r, next_obs, done)]]
        self.n_steps = 0
    ...
    def sample(self, batch_size, seq_len):
        # returns:
        # obs_seq: (B, seq_len+1, obs_dim)
        # act_seq, rew_seq, done_seq: (B, seq_len)
        candidates = [ep for ep in self.episodes if len(ep) >= seq_len]
        if not candidates:
            raise RuntimeError("Not enough episodes/length to sample sequences.")
        ...
        for _ in range(batch_size):
            ...
            # pick start index so that we can take seq_len transitions: [s ... s+seq_len-1]
            s = random.randint(0, len(ep) - seq_len)
            chunk = ep[s:s + seq_len]

            # build obs sequence length seq_len+1: o0 + o1..o_seq_len
            o0 = chunk[0][0]
            obs_seq = [o0] + [tr[3] for tr in chunk]   # next_obs of each transition
...
def train(
    ...
    q = RQNet(obs_dim, act_dim, hidden=rnn_hidden).to(device)
    tq = RQNet(obs_dim, act_dim, hidden=rnn_hidden).to(device)
    tq.load_state_dict(q.state_dict())

    optimizer = optim.Adam(q.parameters(), lr=lr)
    huber = nn.SmoothL1Loss()
    rb = EpisodeReplayBuffer(buffer_size)
    ...
    try:
        for ep in range(1, total_episodes + 1):
            ...
            episode_traj = []  # store transitions for this episode
            ...
            while not done:
                ...
                # always advance hidden with current obs
                obs_t = torch.as_tensor(obs, dtype=torch.float32, device=device).unsqueeze(0)
                with torch.no_grad():
                    qvals, h_next = q.forward_last(obs_t, h)
                ...
                episode_traj.append((obs, int(action), float(reward), next_obs, float(done)))
                ...
                if global_step >= learning_starts and len(rb) >= batch_size * seq_len:
                    obs_seq, act_seq, rew_seq, done_seq = rb.sample(batch_size, seq_len)

                    obs_seq = torch.as_tensor(obs_seq, dtype=torch.float32, device=device)           # (B, T+1, D)
                    act_seq = torch.as_tensor(act_seq, dtype=torch.int64, device=device)            # (B, T)
                    rew_seq = torch.as_tensor(rew_seq, dtype=torch.float32, device=device)           # (B, T)
                    done_seq = torch.as_tensor(done_seq, dtype=torch.float32, device=device)         # (B, T)

                    q_all, _ = q(obs_seq, None)                                                     # (B, T+1, A)
                    qsa = q_all[:, :-1, :].gather(2, act_seq.unsqueeze(-1)).squeeze(-1)              # (B, T)

                    with torch.no_grad():
                        tq_all, _ = tq(obs_seq, None)                                                # (B, T+1, A)
                        next_a = torch.argmax(q_all[:, 1:, :], dim=2, keepdim=True)                  # (B, T, 1)
...
if __name__ == "__main__":
    ...
    p.add_argument("--stack-n", type=int, default=1)       # RNN 비교는 1-step 권장
    ...
    p.add_argument("--batch-size", type=int, default=64)   # 시퀀스라 256보다 줄이는 편이 안전
