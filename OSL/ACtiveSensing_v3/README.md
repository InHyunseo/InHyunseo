...
- **v3 (`OdorHold-v3`)**: Wind-shaped odor plume (advection-like), same obs as v2 (no wind sensor)
...
- Same action/obs format as v2 (per-step: [c, mode], stacked).
- Odor field is a wind-shaped plume (advection-like), controlled by wind vector (wind_x, wind_y).
- No wind sensor in obs (harder; only concentration is observed).
...
- `odor_env_v3.py`  
  **v3** Gymnasium environment (ID: `OdorHold-v3`)  
  Wind-shaped plume (wind_x, wind_y), same obs as v2
...
- v3: `obs_dim = 2 * stack_n` (same as v2)
...
python train.py --env-id OdorHold-v3 --entry-point odor_env_v3:OdorHoldEnvV3 --wind-x 1.0 --wind-y 0.2
```
...
> * v3: `env_id="OdorHold-v3"`, `entry_point="odor_env_v3:OdorHoldEnvV3"`
