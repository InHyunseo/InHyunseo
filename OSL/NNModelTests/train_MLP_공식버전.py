...
def load_ckpt(path, obs_dim, act_dim, device, hidden=256):
    m = DQN(obs_dim, act_dim, hidden=hidden).to(device)
...
def train(
    ...
    plot_ma_window=50,  # kept (you can use it externally if you want)
    video_fps=30,
    video_eval_seed=123,
    ...
    config = dict(
        ...
        force_cpu=force_cpu,
        device=str(device),
        algo="Double DQN (policy argmax + target eval)",
    )
    ...
    policy_net = DQN(obs_dim, act_dim, hidden=256).to(device)
    target_net = DQN(obs_dim, act_dim, hidden=256).to(device)
    ...
    def select_action(state, step):
        ...
        eps_threshold = eps_start + frac * (eps_end - eps_start)
    ...
    def optimize_model():
        ...
        with torch.no_grad():
            # Double DQN: action from policy_net, value from target_net
            if non_final_next_states.numel() > 0:
                next_actions = policy_net(non_final_next_states).max(1).indices.view(-1, 1)
    ...
    # --- plots (keep your metrics) ---
    ep_arr = np.asarray(episodes, dtype=np.int32)

    # raw curves (no numpy moving avg dependency; keep simple)
    save_curves_png(
        run_dir,
        "training_curves.png",
        left_series=[
    ...
    # plot
    plot_durations_png(run_dir, "episode_durations.png", ep_durations, window=100, title="Episode durations")
